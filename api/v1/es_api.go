...
func CreateIndex(ctx *gin.Context) {
	...
	// 判断索引是否存在
	exist, _ := global.ElasticClient.IndexExists(indexName).Do(ctx)
	if exist {
		response.Error(ctx, "索引已经存在，无需重复创建！")
		return
	}
	res, err := global.ElasticClient.CreateIndex(indexName).BodyString(userMapping).Do(ctx)
	if err != nil {
...
func SearchById(ctx *gin.Context) {
	...
	res, err := global.ElasticClient.Get().Index(indexName).Id(id).Do(ctx)
	if err != nil {
