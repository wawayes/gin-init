...
func DeleteUserByUserId(db *gorm.DB, userId string) error {
	return db.Where("user_id = ?", userId).Delete(&User{}).Error
}

// BatchDeleteUserByUserId 批量删除用户
func BatchDeleteUserByUserId(db *gorm.DB, userIds []string) error {
...
func QueryUserByAccount(db *gorm.DB, account string) (*User, error) {
	var user User
...
func QueryUserDetailByUserId(db *gorm.DB, userId string) (*User, error) {
	var user User
...
func QueryUserInfoByUserId(db *gorm.DB, userId string) (*User, error) {
	var user User
	err := db.Select(
		"user_id",
		"user_account",
...
func QueryUserList(db *gorm.DB, page int, size int) ([]*User, error) {
	var users []*User
